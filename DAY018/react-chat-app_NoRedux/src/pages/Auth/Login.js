            ...
            axios
                ...
                .then((res) => {
                    ...
                    if (res.data.code == '200') {
                        // STEP1: 사용자 웹브라우저의 저장공간인 localStorage공간에 서버에서 보내준 사용자인증 jwt 토큰값을 영구보관한다.
                        window.localStorage.setItem('jwttoken', res.data.data)

                        // tip: 사용자 웹브라우저에 저장공간인 localStorage공간에 저장된 데이터를 불러오기
                        const stroageToken = window.localStorage.getItem('jwttoken')
                        console.log('사용자 웹 브라우저에 저장된 JWT사용자 토큰값: ', stroageToken)
                    } else {
                    }
                })
                .catch((err) => {
