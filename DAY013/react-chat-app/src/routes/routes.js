// 리액트 채팅앱의 모든 화면 컴포넌트의 라우팅 규칙을 정의합니다.

import React from 'react'
import { Navigate } from 'react-router-dom'

// React.lazy() 메소드 화면컴포넌트를 동적으로 렌더링하기 위해 사용함
// React.lazy(import(해당화면-페이지컴포넌트 주소))

// 기정의된 페이지 컴포넌트를 참조합니다.

// 사용자인증(로그인) 이 반드시 필요한 화면컴포넌트 목록 참조
const StarterPage = React.lazy(() => import('../pages/Starter/Index'))
const Dashboard = React.lazy(() => import('../pages/dashboard/Index'))
// const Dashboard = React.lazy(() => import('../pages/Starter/Index'))
...
const authProtectedRoutes = [
    ...
    { path: '/', exact: true, component: <Navigate to="/dashboard" /> }, // http://localhost:3000/dashboard
]
