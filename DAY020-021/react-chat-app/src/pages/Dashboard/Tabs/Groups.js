...
// 오리지널 데이터를 인덱스형태로 편집한 구조의 전역변수(배열)
let sortedContacts = [
    {
        group: 'A',
...
const Groups = () => {
    const [config, setConfig] = useState({
        modal: false,
        isOpenCollapse: false, // 멤버목록버튼 클릭시 멤버목록 표시여부
        groups: [],
        selectedContact: [],
        isOpenAlert: false,
        message: '',
    ...
    // DB에서 가져온 회원정보를 A,B,C 단위로 인덱스 형태로 데이터를 재구성해주는 함수
    const sortContact = () => {
    ...
    // 최초 화면 렌더링시에 딱 1번만 오리지널 데이터를 인덱스형 데이터 구조를 변환시킨다.
    useEffect(() => {
        // 모달팝업 회원목록 데이터 바인딩
        sortContact()
        getChannels()
