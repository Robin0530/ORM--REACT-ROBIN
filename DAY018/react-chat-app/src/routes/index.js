...
// 화면 컴포넌트가 렌더링 작업이 끝날때까지 잠시 중단시키고 다른 컴포넌트를 먼저 렌더링하게 하는 기능제공
...
// 로그인여부를 체크한후에 인증되었을때만 보여줘야하는 주소를 호출한경우 로그인 페이지로 반환하는 기능
const AuthProtected = (props) => {
    // 로그인한 사용자 JWT토큰이 없거나 현재 호출하는 페이지가 인증에 의해 보호되는 페이지인 경우 로그인페이지로 이동시킴
...
const Routes = () => {
    return (
        <React.Fragment>
            {/* Suspense를 이용해 해당 태그내에 정의된 모든 개별 라우팅 화면 컴포넌트가 완전 렌더링전에 임시로 보여줄 
            UI 요소를 fallback속성의로 정의한다. */}
            <Suspense fallback={<div>로딩중..</div>}>
                ...
                <SwitchRoute>
                    ...
                    {/* 각각의 페이짘머포넌트를 로그인여부/인증여부를 체크해 비로그인인경우 로그인 페이지로 이동처리하는 로직 추가 적용 */}
