...
// 제너레이터 함수를 노출합니다.
// watchLoginUser함수를 정의하고 기능을 노출하면 추후 관련 액션함수가 실행되면 자동은 saga 제너레이터 함수가 실행됩니다.
...
function* authSaga() {
    // all(배열안에 해당 saga함수를 fork 메소드로 감싸서 배열형태로 여러개 제공[])
    // all함수는 실행해야할 사가함수들을 배열에 넣고 여러개를 동시에 실행시킬 수 있는 제공
    // fork헬퍼함수는 비동기 기반 함수를 실행할 때 사용함
