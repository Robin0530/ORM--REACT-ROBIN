...
// 리듀서 함수는 해당 업무와 관련된 다양한 액션타입별로 전역상태를 관리해주는 기능을 제공합니다.
// 리듀서 함수에 입력파라메터로 action 타입 객체가 전달되고, action 타입 객체안에는 화면 컴포넌트안에서 전달된 데이터가 포함되어 있음
// 해당 업무 전용 리듀서 함수는 다양한 해당업무 액션타입별로 데이터를 case by case로 관리할수 있다.
// state는 store에 저장된 전체 globalState내 todo 전용 전역데이터를 말함
